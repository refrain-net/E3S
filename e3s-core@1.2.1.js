...
function setDate (hour) {
  if (hour < 0 || hour > 23) throw new RangeError();
  hour %= 24;
...
function getTotalTime () {
  return ms2hr(refDate.getTime() - baseDate.getTime()) - 7 * getLoopCount() - disableTime + baseTime;
}

/**
 * @function getLoopCount 繰り返し回数を取得する
 * @return {number} 加算される繰り返し回数 + 繰り返し回数の補正値
 */
 ...
 * @return {string[]} [YYYY/MM/DD, hh:mm]形式に変換されたrefDate
 */
